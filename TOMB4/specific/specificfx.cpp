...
void DoRain()
{
	...
	for (int i = 0; i < rain_count; i++)
	{
		...
		if (rain_outside && !rptr->x && num_alive < max_rain)
		{
			...
			if (IsRoomOutside(rptr->x, rptr->y, rptr->z) < 0 && WeatherType == 1)
			{
				rptr->x = 0;
				continue;
			}
			...
			if (!(room[IsRoomOutsideNo].flags & ROOM_RAIN) && WeatherType != 1)
			{
				rptr->x = 0;
				continue;
			}
...
void DoSnow()
{
	...
	for (int i = 0; i < snow_count; i++)
	{
		...
		if (!snow->x)
		{
			...
			if (IsRoomOutside(snow->x, snow->y, snow->z) < 0 && WeatherType == 2)
			{
				snow->x = 0;
				continue;
			}
			...
			if (!(room[IsRoomOutsideNo].flags & ROOM_SNOW) && WeatherType != 2)
			{
				snow->x = 0;
				continue;
			}
