...
void InitialiseChef(short item_number)
{
	...
	int16_t chef_slot = T4PlusGetChefSlotID();
	if (chef_slot < 0 || chef_slot >= NUMBER_OBJECTS) {
		return;
	}

	item = &items[item_number];
	InitialiseCreature(item_number);
	item->anim_number = objects[chef_slot].anim_index;
	item->frame_number = anims[objects[chef_slot].anim_index].frame_base;
